...
import { AgreeTermsPolicyRequestComponent } from './agree-terms-policy-request/agree-terms-policy-request.component';
  ...
  onSubmit(form: NgForm) {
    ...
    this.authService.login(this.loginData).subscribe({
      next: (response: JwtResponse) => {
        ...
        // Palestinian users → check terms‐agreement
        this.pendingRoute = '/dashboard';
        this.userService.getAgreeStatus(response.shayyikliAccountNumber).subscribe({
          next: (agreed: boolean) => {
            this.loadingService.loadingOff();
            if (agreed) {
  ...
  validateEmail(ctrl: NgModel) {
    const v = ctrl.value ?? '';
    this.emailError = v && !/^[\w.%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$/.test(v)
